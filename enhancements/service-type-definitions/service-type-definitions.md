...
| :------------- | :------- | :------------------------------------- | :--------------------------------------------------------------------------------------- | :------- |
| service_type   | Yes      | string                                 | Service type identifier (_vm_, _container_, _database_, _cluster_, _storage_, _network_) | No       |
...
Specifies whether traffic is handled at the transport level (TCP/UDP) or the
application level (HTTP/HTTPS). When omitted, creates a basic service without
external routing infrastructure.

- **Default:** omitted (no external routing)
- **Values:**
  - `network`: Transport-level traffic handling (TCP/UDP)
  - `application`: Application-level traffic handling (HTTP/HTTPS) with routing
    capabilities
...
| nodePorts | No       | map[string]int    | Map of port names to NodePort values (30000-32767). The Kubernetes Service type is inferred from the combination of `routingLevel` and `nodePorts` presence. See [Service Type Inference](https://github.com/dcm-project/enhancements/blob/main/enhancements/k8s-network-sp/k8s-network-sp.md#service-type-inference). |
