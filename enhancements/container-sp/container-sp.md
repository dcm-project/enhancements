...
- Define endpoints for day 2 operations (`restart`, `scale`, `update`) for
  container instances.
...
| GET    | /api/v1/health                         | Container API service health check |
...
namespace via the `metadata.namespace` field.
...
namespace, status, ip, ports) to match the detail level of the GET single
resource endpoint.
