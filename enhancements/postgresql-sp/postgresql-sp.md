...
> **Note:** The `init.connectionDetails` field is empty at creation time and
> will be populated once the `Cluster` reaches `READY` status. The
> `connectionDetails` field will contain a base64-encoded version of the
> database application connection details, including user credentials The user's
> password field would not be included in the response as it is specified in the
...
The `initdb.connectionDetails` field is populated based on the cluster status:
...
The `initdb.connectionDetails` field follows the schema of the <cluster>-app
secret's format that is generated in the case no password is specified. If a
password is specified, the PostgreSQL will generate the rest of the fields by
querying the cluster and aggragating with user configuration. The details in the
`initdb.connectionDetails` take on the following structure:
...
The `initdb.connectionDetails` field contains sensetive credentails that grant
access to the provisioned Cluster. Implementations should:
...
In the case of Multiple replicas, the state is aggragated in the following
logic:

Each replica is created as a set of `Pod` and `PVC` resources. We can
distinguish all replicas of the same cluster by 2 labels:
...
replica's state by the following table:
...
> **Note**: In this context, replica is used to refer to both primary and
...
The replica's status is aggragated into a single DCM status using the following
mapping:

| DCM Status | Primary Source | Status                                                                                                                               | Precedence |
| ---------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| PENDING    | Any replica    | any replica PENDING AND (no replica FAILED OR UNKNOWN)                                                                               | 1          |
...
| RUNNING    | All replicas   | all replicas RUNNING                                                                                                                 | 1          |
| FAILED     | Any replica    | any replica FAILED                                                                                                                   | 1          |
| UNKNOWN    | Any replica    | Any replica UNKNOWN AND no replica FAILED                                                                                            | 1          |
| DELETED    | All            | All replicas DELETED and Cluster not found                                                                                           | 3          |
...
- **1 (Replica)**: Highest priority - report Replica state if replica exists
